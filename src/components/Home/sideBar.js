...
const SideBar = () => {
  ...
  const changeNavbarColor = () => {
    if (window.scrollY >= 50 && window.scrollY <= 2300) {
      setColorchange(classes.nav__cont1);
      console.log("more than 50 and less than 200");
    } else if (window.scrollY > 2300) {
      setColorchange(classes.nav__cont2);
      console.log("more than 200");
    } else {
      setColorchange(classes.nav__cont);
    }
  };
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <nav class={colorChange ? colorChange : classes.nav__cont}>
